...
def buildProjectWithCheribuild(String projectName, String extraArgs, String targetCPU) {
    ...
    stage("build ${targetCPU}") {
        ...
        sh 'ls -la'
        archiveArtifacts allowEmptyArchive: false, artifacts: "${projectName}-${targetCPU}.tar.xz", fingerprint: true, onlyIfSuccessful: false
    }
}
...
stage("Build") {
    parallel cheribuildProject('postgres')
}
